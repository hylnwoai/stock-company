...
code_UK=[]   #10123
name_UK=[]
f=open("C:/Users/HL-lisa/Desktop/England.txt","r")
for line in f:
    code=line.split("||")[0]
    code_UK.append(code)
    name_UK.append(line.split("||")[1].strip())
f.close()
code_UK=list(set(code_UK))
...
class odd(threading.Thread):
    ...
    def run(self):
        for i in range(1,len(code_UK), 2):
            try:
                print("第" + str(i) + "页")
                url = "http://basic.10jqka.com.cn/160/" + code_UK[i] + "/company.html"
                page = requests.get(url, headers=headers, proxies=random.choice(https_proxies))
                page.encoding='gbk'
                print("网页长度：" + str(len(page.text)))
                ...
                if all([t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11]):
                    print("股票代码:" + code_UK[i])
                    code.append(code_UK[i])
                    print("股票名称：" + name_UK[i])
                    name.append(name_UK[i])
                    print("logo:" + t1)
                    logo.append(t1.strip())
                    print("公司名称：" + t2)
                    company_name.append(t2.strip())
...
class even(threading.Thread):
    ...
    def run(self):
        for i in range(0,len(code_UK), 2):
            try:
                print("第" + str(i) + "页")
                url = "http://basic.10jqka.com.cn/160/" + code_UK[i] + "/company.html"
                page = requests.get(url, headers=headers, proxies=random.choice(https_proxies))
...
import pandas as pd
data={'code':code,'name':name,'logo':logo,'company_name':company_name,'abbr_name':abbr_name,'english_name':english_name,'stock_exchange_name':stock_exchange_name,'industry':industry,'homepage':homepage,'people':people,'register_address':register_address,'address':address,'description':description}
data_UK=pd.DataFrame(data)
data_UK.to_excel('C:/Users/HL-lisa/Desktop/stock_UK_company.xlsx',index=False)
